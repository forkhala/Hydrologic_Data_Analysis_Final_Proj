...
pb <- winProgressBar(title="Extracting pixels of agricultural lands", label="0% done", 
                     min=0, max=100, initial=0)
for (i in 1:10) {
  pasture.raster.temp <- landcover.raster.list[[i]] == 557
  assign(paste0("pasture.raster.", raster.list.nm[[i]]), pasture.raster.temp)
  rm(pasture.raster.temp)
...
#pasture
pasture.raster.allstates <- 
  raster::merge(pasture.raster.co,pasture.raster.ia,pasture.raster.ks,pasture.raster.mn,
                pasture.raster.mo,pasture.raster.mt,pasture.raster.nd,pasture.raster.ne,
                pasture.raster.sd,pasture.raster.wy, overlap = TRUE,
                filename = "../Untracked Proj Data/Land_Cover/pasturemap_allstates_high.tif")
plot(pasture.raster.allstates)
...
gdalUtilities::gdalwarp(f, "../Untracked Proj Data/Land_Cover/cropmap_allstates_low.tif",
                        r='mode', multi=TRUE, tr=res(crop.raster.allstates)*100)

crop.raster.low.allstates <- raster("../Untracked Proj Data/Land_Cover/cropmap_allstates_low.tif")
plot(crop.raster.low.allstates)

#pasture
writeRaster(pasture.raster.allstates, f <- tempfile(fileext='.tif'), datatype='INT1U')
gdalUtilities::gdalwarp(f, "../Untracked Proj Data/Land_Cover/pasturemap_allstates_low.tif",
                        r='mode', multi=TRUE, tr=res(pasture.raster.allstates)*100)

pasture.raster.low.allstates <- raster("../Untracked Proj Data/Land_Cover/pasturemap_allstates_low.tif")
plot(pasture.raster.low.allstates)
...
pasture.raster.map.allstates <- projectRaster(pasture.raster.low.allstates, 
                                           crs = crs, method = "ngb")
structure(pasture.raster.map.allstates)
...
crop.sf.allstates <- st_read("./Data/Shapefiles/Land_Cover/cropland.shp")
...
#pasture
system.time(polygonizer(pasture.raster.map.allstates, aggregate = T, readpoly = F,
                        outshape = "./Data/Shapefiles/Land_Cover/pasture"))

pasture.sf.allstates <- st_read("./Data/Shapefiles/Land_Cover/pasture.shp")
st_crs(pasture.sf.allstates)

pasture.map <- pasture.sf.allstates %>%
  filter(DN == 1) %>%
  mutate(land = if_else(DN == 1, "Pasture", "NA")) %>%
  dplyr::select(-DN)

### Manually set colors
colornames <- c(brewer.pal(11, "Paired"), "red", "burlywood3", "yellowgreen")
variables <- c(HUC4.SE$Name, "Impaired Waters", "Cropland", "Pasture")
names(colornames) <- variables

cropplot1 <- ggplot() +
  geom_sf(data = allstates.map, fill = "white", size = 0.4) +
  geom_sf(data = cropland.map, aes(fill = land), alpha=0.4, linetype = 0) +
  geom_sf(data = pasture.map, aes(fill = land), alpha=0.4, linetype = 0) +
  geom_sf(data = HUC4.SE, aes(fill = Name), alpha = 0.3, size = 0.45) +
  geom_sf(data = HUC4.NW, fill = "grey85", alpha = 0.1, size = 0.45) +
  geom_sf(data = streams.HU10, color = "lightskyblue2", alpha = 0.8, size = 0.4) +
  geom_sf(data = missouri, color = "dodgerblue2", alpha = 0.75, size = 0.7) +
  ...
  geom_sf(data = best.sites.spatial, fill="black", color="black", 
          alpha = 0.8, size = 1.15) +
  scale_fill_manual(values = colornames,                                # mannually set colors
                    name = "Watershed Names and \n Landscape Features", #
                    limits=variables) +                                 # set order of legend items
...
cropplot2 <- ggplot() +
  ...
  geom_sf(data = best.sites.spatial, fill="black", color="black", 
          alpha = 0.8, size = 1.15) +
  scale_fill_manual(values = colornames[12:14], name = "", # mannually set colors
                    limits=c("Impaired Waters", "Cropland", "Pasture")) + # set order of legend items
  labs(x = element_blank(), y = element_blank())+
  geom_sf_text_repel(data = best.sites.spatial, aes(label = site_lab),
                     force = 1.5, box.padding = 0.30, min.segment.length = 0.4)+
  theme(legend.margin = margin(0,0,0,0, "pt"), legend.text = element_text(size = 7.5),

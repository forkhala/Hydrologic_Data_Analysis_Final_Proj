...
```{r}
#Datatable stuff
bestsites.DNP <- read.csv("./Data/Raw/bestsites.DNP.csv", as.is=TRUE)
bestsites.DNP$Date <- as.Date(bestsites.DNP$Date)

range.P <- bestsites.DNP %>%
  filter(parm_cd=="665") %>%
  summarise(minDate = min(Date),
            maxDate = max(Date),
            Count = length(parm_cd),
            minva = min(result_va), 
            maxva = max(result_va))

range.D <- bestsites.DNP %>%
  filter(parm_cd=="60") %>%
  summarise(minDate = min(Date),
            maxDate = max(Date),
            Count = length(parm_cd),
            ...
            maxva = max(Discharge))
range.D$minva <- "0"
range.D$maxva <- "739000"

range.N <- bestsites.DNP %>%
  filter(parm_cd=="600") %>%
  summarise(minDate = min(Date),
            maxDate = max(Date),
            Count = length(parm_cd),
...
range.table <- rbind(range.D, range.N, range.P)
range.table$Parameter <- c("Discharge", "Nitrogen", "Phosphorus")
range.table$unit <- c("cfs", "mg/L", "mg/L")
range.table$TypeSites <- c("daily and high frequency", "daily and high frequency", "daily")
range.table <- range.table %>% dplyr::select(Parameter, unit, minDate, maxDate, minva, maxva, Count, TypeSites)
write.csv(range.table, "./Data/Processed/range.table.csv")

bestsites.table <- bestsites.countypop.info %>%
  dplyr::select(site_lab, site_no, site_nm, huc4, state, county_name, population) %>%
  arrange(site_lab)

bestsites.table <- as.data.frame(bestsites.table)
bestsites.table <- bestsites.table %>% dplyr::select(-geometry)
names(bestsites.table) <- c("No.", "Site Number", "Site Name", "HUC 4 Region", "State", "County Name", "County Population")
write.csv(bestsites.table, "./Data/Processed/bestsites.table.csv")
```
...
kable(bestsites.table, format = "latex", booktabs = T,
      ...
      caption = "\\label{tab:bestsitestable} Caption please finish") %>%

...
To reveal whether discharge has become more variable over time, we analyzed the relationship between standard deviation (SD) of discharge within a year and across years, using linear models (\autoref{fig:varfig}; \autoref{tab:vartab}). Among all the 22 sites, 14 sites suggest increasing SD over time, and four of them show statistically significant increase. By contrast, only eight sites suggest declining SD trends, and two of them exhibit statistically significant increased SD, or more variability. Despite that only data from six sites (around 27%) yield conclusive results, there are twice as many streams with increasing variability than with decreasing variability.  Thus, our results suggest that the whole Missouri Basin appear to have become more variable over years since around the mid-19th century. The low percentage of significant results could result from the small effect size, and/or the short time span of available data for some sites. Hence, more monitoring on discharge in the basin are required in the future to reach a more definitive conclusion.

```{r varfig, echo=FALSE, warning=FALSE, out.width="\\maxwidth", fig.cap="\\label{fig:varfig} Exploratory analysis examining the changes in the standard deviations (SD) of discharge within a year at 22 sites over the whole time span of available data. Red regression lines and confidence intervals suggest increasing SD, whereas blue for decreasing SD over time. Bolded site labels and darker regression lines indicate significant results, whereas regression lines in gray are non-significant ($\\alpha = 0.05$)."}

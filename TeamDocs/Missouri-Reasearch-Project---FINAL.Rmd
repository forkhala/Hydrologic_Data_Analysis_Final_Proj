...
## Changes in Discharge Variation over Years
To reveal how variability of discharge has changed over time, we first used linear model to analyze the relationship between standard deviation (SD) of discharge within a year and years (\autoref{fig:sdfig}; \autoref{tab:vartab}). Among all the 22 sites, 14 sites seemed to increase in SD over time, and 4 of them showed statistically significant increase. By contrast, only 8 sites had declining SD trends, and 2 of them exhibited statistically significant rising in SD. Despite that only data from six sites (around 27%) yielded conclusive results, within these sites the number of streams that had higher SD was two times as many as those with decreasing SD. Thus, our results suggest that the whole Missouri Basin appears to have increased in absolute variability over years since around the mid-19th century. The low percentage of significant results could result from the small effect size, and/or the short time span of available data for some sites. 

```{r sdfig, echo=FALSE, warning=FALSE, out.width="\\maxwidth", fig.cap="\\label{fig:varfig} Changes in the standard deviations (SD) of discharge within a year at 22 sites over the whole time span of available data. Red regression lines and confidence intervals represent increasing SD, whereas blue lines for decreasing SD over time. Bolded site labels and HUC8 with asterisk represent significant results, while regression lines in gray color are non-significant ($\\alpha = 0.05$)."}
...
```{r cvfig, echo=FALSE, warning=FALSE, out.width="\\maxwidth", fig.cap="\\label{fig:varfig} Changes in the coefficient of variation (CV) of discharge at 22 sites over the whole time span of available data. Red regression lines and confidence intervals represent increasing CV, whereas blue lines for decreasing CV over time. Bolded site labels and HUC8 with asterisk represent significant results, while regression lines in gray color are non-significant ($\\alpha = 0.05$)."}
...
The miniumum 7 day average flow (cfs) varied for each river and between years in rivers in the same river (\autoref{fig:MinFlowMill}). 
